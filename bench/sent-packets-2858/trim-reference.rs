...
/// Sorted packet numbers with bounded tombstones.
///
/// Packet-number gaps never allocate padding. Interior removals leave tombstones,
/// and compaction keeps storage proportional to the live window plus a constant
/// allowance. Individual cleanup and shrink operations can still take O(n) time.
#[derive(Default)]
...
impl SentPackets {
    /// Insert a packet whose number exceeds every previously inserted number.
    ...
    pub(super) fn remove(&mut self, pn: u64) -> Option<SentPacket> {
        ...
        self.in_flight -= usize::from(value.size != 0);
        self.live -= 1;
        // Removing the newest retained slot can expose a suffix of tombstones.
        // Reclaim it without moving any live packets or waiting for compaction.
        if index + 1 == self.packets.len() {
            while self.packets.back().is_some_and(|x| x.1.is_none()) {
...
#[cfg(test)]
#[test]
fn reclaim_deleted_suffix_with_old_anchor() {
    let mut packets = SentPackets::default();
    packets.insert(0, trim_test_packet(0));
    for pn in 1..=1000 {
        packets.insert(pn, trim_test_packet(pn as u16));
    }
    for pn in 700..1000 { assert!(packets.remove(pn).is_some()); }
    assert!(packets.remove(1000).is_some());
    assert_eq!(packets.packets.back().unwrap().0, 699);
    assert_eq!(packets.live, 700);
    for pn in (1..700).rev() { assert!(packets.remove(pn).is_some()); }
    assert_eq!(packets.packets.len(), 1);
    assert_eq!(packets.get(0).unwrap().size, 0);
    packets.insert(1_000_000, trim_test_packet(42));
    assert_eq!(packets.range(..).map(|(pn, _)| pn).collect::<Vec<_>>(), vec![0, 1_000_000]);
}

#[cfg(test)]
fn trim_test_packet(size: u16) -> SentPacket {
    SentPacket {
        path_generation: 0, time_sent: crate::Instant::now(), size,
        ack_eliciting: size != 0, largest_acked: None,
        retransmits: Default::default(), stream_frames: Default::default(),
    }
}
